...
class __TwigTemplate_782c1c01b51d3a57ffbf167fd0e2c01e extends Template
{
    ...
    public function block_table_renders(array $context, array $blocks = []): iterable
    {
        ...
        foreach ($context['_seq'] as $context["_key"] => $context["render"]) {
            ...
            yield "</td>
                    ...
                    ";
            // line 296
            if (CoreExtension::getAttribute($this->env, $this->source, $context["render"], "component", [], "any", true, true, false, 296)) {
                // line 297
                yield "                    <tr>
                        <th scope=\"row\">Component</th>
                        <td colspan=\"4\">";
                // line 299
                yield $this->extensions['Symfony\Bundle\WebProfilerBundle\Twig\WebProfilerExtension']->dumpData($this->env, CoreExtension::getAttribute($this->env, $this->source, $context["render"], "component", [], "any", false, false, false, 299));
                yield "</td>
                    </tr>
                    ";
            }
...
{% block page_title \x27Twig Components\x27 %}
...
{% block toolbar %}
    {% if collector.renderCount %}
        ...
        {% set icon %}
            {{ source(\x27@TwigComponent/Collector/icon.svg\x27) }}
...
{% block menu %}
    <span class=\"label{{ collector.components is empty ? \x27 disabled\x27 }}\">
        <span class=\"icon\">{{ source(\x27@TwigComponent/Collector/icon.svg\x27) }}</span>
        <strong>Twig Components</strong>
    </span>
{% endblock %}
...
{% block panel %}
    ...
    {% else %}
        <section class=\"twig-component-metrics metrics\">
            <div class=\"metric-group\">
                {{ _self.metric(collector.componentCount, \x27Twig Components\x27) }}
            </div>
            <div class=\"metric-divider\"></div>
            <div class=\"metric-group\">
                {{ _self.metric(collector.renderCount, \x27Render Count\x27) }}
                {{ _self.metric(collector.renderTime|round, \x27Render Time\x27, \x27ms\x27) }}
            </div>
            <div class=\"metric-divider\"></div>
            <div class=\"metric-group\">
                {{ _self.metric((collector.peakMemoryUsage / 1024 / 1024)|number_format(1), \x27Memory Usage\x27, \x27MiB\x27) }}
            </div>
        </section>
        <section class=\"twig-component-components\">
            <h3>Components</h3>
            {{ block(\x27table_components\x27) }}
        </section>
        <section class=\"twig-component-renders\">
            <h3>Render calls</h3>
            {{ block(\x27table_renders\x27) }}
        </section>
    {% endif %}
{% endblock %}

{% macro metric(value, label, unit = \x27\x27) %}
    <div class=\"metric\">
        <span class=\"value\">
            {{ value }}
            {% if unit %}
        ...
        <tbody>
            {% for component in collector.components %}
                <tr>
                    ...
                    <td>
                        {% if component.class == \x27Symfony\\\\UX\\\\TwigComponent\\\\AnonymousComponent\x27 %}
    ...
    <div class=\"twig-component-renders\">
        ...
        {% for render in collector.renders %}
            <table class=\"twig-component-render\" style=\"--render-depth:{{ render.depth }};\">
                <thead
                    class=\"sf-toggle {{ loop.index == 1 ? \x27sf-toggle-on\x27 : \x27sf-toggle-off\x27 }}\"
                   data-toggle-selector=\"#render-{{ loop.index }}--details\"
                   data-toggle-initial=\"{{ loop.index == 1 ? \x27display\x27 }}\"
                >
                    <tr>
                        <th class=\"key\">{{ render.depth ? source(\x27@TwigComponent/Collector/chevron-down.svg\x27) }}{{ render.name }}</th>
                        <th>
                            {% if render.class == \x27Symfony\\\\UX\\\\TwigComponent\\\\AnonymousComponent\x27 %}
                                <pre class=\"sf-dump\"><span class=\"text-muted\">[Anonymous]</span></pre>
                            {% else %}
                                {{ render.class }}
                            {% endif %}
                        ...
                        <th class=\"cell-right renderTime\">
                            ...
                            <span class=\"{{ _render_memory == _memory ? \x27text-muted\x27 }}\">
                        ...
                        <th class=\"cell-right\">
                            <button class=\"btn btn-link toggle-button\" type=\"button\" aria-label=\"Toggle details\">
                                {{ source(\x27@TwigComponent/Collector/chevron-down.svg\x27) }}
                            </button>
                        </th>
                    </tr>
                </thead>
                <tbody id=\"render-{{ loop.index }}--details\">
                    <tr class=\"{{ not render.input_props|default ? \x27opacity-50\x27 }}\">
                        <th scope=\"row\">Input props</th>
                        <td colspan=\"4\">{{ profiler_dump(render.input_props) }}</td>
                    </tr>
                    <tr class=\"{{ not render.attributes|default ? \x27opacity-50\x27 }}\">
                        <th scope=\"row\">Attributes</th>
...
{% endblock %}
", "@TwigComponent/Collector/twig_component.html.twig", "/Users/diabiraaly/Desktop/Bureau - MacBook Air de DIABIRA/Openclassrooms/OC14/vendor/symfony/ux-twig-component/templates/Collector/twig_component.html.twig");
    }
}
